...
### Проверенный совет с применением и отменой

`POST /api/advice` принимает `{ scenario: ScenarioInput }` и возвращает `{ proposal: { baseScenario, scenario } | null, explanation: string, source: "openai" | "fallback" }`. Это отдельный контракт от `/api/analyze`.

Сервер перебирает одиночные замены через `suggestSwaps`, выбирает улучшение отображаемого Score без роста расходов и возвращает точный сценарий. Модель объясняет уже выбранную замену; при ошибке остаётся тот же совет с детерминированным объяснением. Если улучшения нет, `proposal` равен `null`.

`compareAdvice` проверяет свежесть исходного сценария, ровно одно изменённое решение, ограничения, расходы и улучшение Score; числа до/после всегда пересчитываются локально. `AdviceCard` показывает замену, числа и потери районов/показателей. `page.tsx` владеет рассчитанным сценарием и одним шагом отмены. `ScenarioBuilder` принимает `initialScenario`, сообщает `onChange` при ручной правке и передаёт `(result, scenario)` в `onSimulate`; при применении/отмене родитель меняет `key`, чтобы обновить все пять слотов. Ручная правка скрывает старый результат и совет, отменяет ожидающий запрос и сбрасывает отмену.
