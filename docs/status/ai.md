# Статус: ai

| ID | Задача | Дедлайн | Статус | Начато | Готово | Комментарий |
|---|---|---|---|---|---|---|
| A1 | Next Route Handler POST /api/analyze: рассчитанный JSON → AI brief | 15:00 | done | 16:50 | 17:08 | Контекст для LLM локализован и округлён; числовые факты формирует приложение; технические имена и числа в прозе отклоняются. 103 теста, lint, build и live OpenAI-проверка прошли. |
| A2 | Детерминированный fallback без LLM-ключа | 15:30 | done | — | 15:09 | Готовый MVP; перенесено в TS-план по решению PM. |
| I1 | ИНТЕГРАЦИЯ расширенного демо на одной машине, окно 16:50–17:00 | 17:00 | todo | — | — |  |
| A4 | Подключение агента к UI + тюнинг промптов на демо-вопросах | 16:50 | todo | — | — |  |
| F0 | ФРИЗ КОДА: после 17:20 пушит только ui (README) | 17:20 | todo | — | — |  |
| R2 | Репетиция демо | 17:55 | todo | — | — |  |

A3 → engine, done (начато 16:35, готово 16:42), по решению PM; актуальная строка — docs/status/engine.md.

Для U4: POST /api/agent, body {mode, message, scenario} → {answer, trace, mode}; демо-вопросы — src/lib/agent/demo-questions.json

Интеграция скриншотов и панелей по запросу PM: done, Начато 17:48, Готово 17:51 — шесть скриншотов подключены в README; ключи advice/agent различаются и сохраняют сброс при смене сценария. В браузере лучший набор рассчитан повторно: по одной панели, новых ошибок консоли нет. 127 тестов, lint и build прошли.

Финальные исправления по прямому запросу PM: done, Начато 17:40, Готово 17:41 — полная валидация входа записки, серверный таймаут 8 с, клиентский 12 с и каталог названий для агента. Совместные проверки с UI: 127 тестов, lint и production build прошли.
